...
export interface IndexedDomain {
  ...
  bimi: { present: boolean; hasVmc: boolean; declined: boolean };
  mtaSts: { present: boolean; mode: string | null };
  tlsRpt: { present: boolean };
  mx: { provider: string | null; isNullMx: boolean; hosts: string[] };
  dkim: { selectorsFound: string[]; probeStrategy: string };
}
